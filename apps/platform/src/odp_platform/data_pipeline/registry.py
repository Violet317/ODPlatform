...
def register(
    format_name: str,
    supported_tasks: Tuple[str, ...] = ("detect",),
    ...
    def decorator(func: ConverterFunc) -> ConverterFunc:
        ...
        logger.debug(
            f"注册 converter: format={format_name}, tasks={supported_tasks}"
        )
        return func
    return decorator
...
def _lazy_init() -> None:
    ...
    延迟加载原因: 避免循环 import (core/* 模块会 from registry import register)。
    自动发现: 用 pkgutil.iter_modules 扫描包目录, 新增 core/<format>.py 后
    什么都不用改, 文件存在即自动注册。
    """
